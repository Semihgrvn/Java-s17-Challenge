...
public class CourseValidation {
    ...
    public static void isDuplivateNameFound(List<Course> courses, String name) {
        Optional<Course> foundCourse = courses.stream().filter(c -> c.getName().equals(name)).findFirst();
        if (foundCourse.isPresent()) {
            throw new CourseException("Course with tihs name already exist", HttpStatus.BAD_REQUEST);
        }
    }
}
